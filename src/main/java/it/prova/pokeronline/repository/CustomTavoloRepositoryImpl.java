...
public class CustomTavoloRepositoryImpl implements CustomTavoloRepository {
	...
	public List<Tavolo> findByExample(Tavolo example, Utente utente) {
		...
		if (utente != null) {
			...
			paramaterMap.put("utenteCrezione", utente.getId());
		}
		...
		if (example.getUtenteCreazione().getId() != null) {
...
//		if (StringUtils.isNotEmpty(example.getUtenteCreazione().getUsername())) {
//			whereClauses.add(" t.utenteCreazione.username :username ");
//			paramaterMap.put("username", example.getUtenteCreazione().getUsername());
//		}
